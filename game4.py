...
class Game4():
    def __init__(self):
        ...
        self.secret_word = words.english_4L[random.randint(0, len(words.english_4L) - 1)]
    ...
    def draw_board(self, size):
        ...
        for col in range(0, 4):
            for row in range(0, 6):
                pygame.draw.rect(self.display, self.WHITE, [50 + col * 100 + 12, row * 100 + 12, 75, 75], 3, 5)  # squares around letter
                piece_text = font.render(self.board[row][col].upper(), True, self.LETTER)  # upper case when printing on screen
                self.display.blit(piece_text, (50 + col * 100 + 30, row * 100 + 25))  # draw the text into the screen
        if not self.game_over:  # do not show selector if game is over
            pygame.draw.rect(self.display, self.SELECTOR, [50, self.turn * 100 + 5, self.DISPLAY_W - 100, 90], 3, 5)
    ...
    def check_words(self):
        for row in range(0, self.turn):
            ...
            # build color array
            letter_colors = [self.LETTER_INCORRECT] * 4
            ...
            # draw colors
            for col in range(0, 4):
                pygame.draw.rect(self.display, letter_colors[col], [50 + col * 100 + 12, row * 100 + 12, 75, 75], 0, 5)
    ...
    def check_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN and not self.game_over and self.letters == 4:  # restrict to 4 letters to move to next line
                    if self.in_wordlist():
    ...
    def in_wordlist(self):
        ...
        return the_word in words.english_4L
